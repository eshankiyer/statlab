...
export function ChiSquareCalculator() {
  ...
  const calculateArea = (x: number) => {
    return jstat.chisquare.cdf(x, Number(df))
  }

  const calculateInverse = (p: number) => {
    return jstat.chisquare.inv(p, Number(df))
  }
  ...
  const generateChartData = () => {
    ...
    for (let i = 0; i <= numDf * 3; i += numDf / 20) {
      data.push({
        x: i,
        y: jstat.chisquare.pdf(i, numDf),
      })
    }
    return data
  }
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid gap-2">
          ...
          <Select value={operation} onValueChange={setOperation}>
            ...
            <SelectContent>
              <SelectItem value="area">Calculate area under the χ² curve</SelectItem>
              <SelectItem value="inverse">Calculate inverse of χ² distribution</SelectItem>
            </SelectContent>
          </Select>
        </div>

        <div className="grid gap-2">
          <label htmlFor="value">{operation === "area" ? "χ² value" : "Probability"}</label>
